...
//создадим класс для анализа ключевых слов
public abstract class KeywordAnalyzer implements TextAnalyzer{

    protected abstract String[] getKeywords(); // абстрактный метод, который который будет возвращать набор ключевых слов

    protected abstract Label getLabel(); //метод, который будет возвращать метку, которой необходимо пометить положительные срабатывания

    @Override
    public Label processText(String text) {
        for (String s : getKeywords()) { //проходимся по массиву ключевых слов
            if (text.matches("(.*)" + s + "(.*)")) { //с помощью matches проверяем содержит ли строка ключевое слово, регулярное выражение указанное в скобках означает, что  перед и после ключевого слова может стоять любое количество символов
                return getLabel();} //и возвращаем метку
        }
